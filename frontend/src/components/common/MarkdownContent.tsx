  ...
  return (
    <div className={`prose prose-invert max-w-none ${className}`}>
      <ReactMarkdown
        ...
        components={{
          p: ({ children }) => <p className="text-sm leading-relaxed text-white mb-2 last:mb-0">{children}</p>,
          ...
          li: ({ children }) => <li className="text-sm leading-relaxed text-gray-200">{children}</li>,
          img: ({ src, alt }) => (
            <img
              src={src}
              alt={alt ?? ''}
              className="inline-block align-text-bottom w-4 h-4 md:w-[18px] md:h-[18px] mx-1 my-0 rounded-sm border border-white/10 object-contain"
              loading="lazy"
              onError={e => {
